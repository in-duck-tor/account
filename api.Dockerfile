...
RUN dotnet publish -c Release -o ./publish/ --no-restore ./src/InDuckTor.Account.WebApi/InDuckTor.Account.WebApi.csproj

FROM mcr.microsoft.com/dotnet/aspnet:8.0 as runtime
VOLUME /app/certs
WORKDIR /app
COPY --from=build /app/publish .
ENV OTEL_RESOURCE_ATTRIBUTES="service.name=account-api"
